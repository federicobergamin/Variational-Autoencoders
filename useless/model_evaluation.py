...
PATH = 'VAE_personal_implementation/saved_models/VAE_zdim_3_epoch_100_elbo_-137-1356429361979_learnrate_0-001_Andrea'
...
with torch.no_grad():
    for i, data in enumerate(test_loader, 0):
        ...
        # batch_marginal = math.log(0.5 * len(images)) + torch.logsumexp(likelihood - kl, dim=0)
        batch_marginal_log = math.log(1 / len(images)) + torch.logsumexp(likelihood - kl, dim=0)
        # print('batch marginal', batch_marginal/len(images))

        marginal_log_likelihood += batch_marginal_log
